/* ============================================================
   ...
   Update: Mei 2026 — Marketplace Mode Prices
   - Material aktual: LONGi Hi-MO X10 650Wp, Powmr Hybrid MPPT,
     Shoto LiFePO4, Tomzn MCB DC
   - HPP based on harga marketplace (Tokopedia/Shopee) + ongkir ke Jambi
   - Panel wattage: 650Wp (LONGi Hi-MO X10 TOPCon)
   - PSH Jambi: 3.75 jam | Efficiency: 80%
   - Margin: 35% | PPN: 11%
   - ROI honest: ~8 tahun off-grid (flat tariff), ~6-7 thn (with PLN increase)
   - Battery options available for ALL packages (not just Gold/Platinum)
   ============================================================ */
...
export interface PackageSpec {
  ...
  popular: boolean;
  batteryOptions?: number[];  // Available battery kWh options for this package
}

// Battery options per tier
export const BATTERY_OPTIONS_SILVER = [0, 3, 5, 7];
export const BATTERY_OPTIONS_GOLD = [0, 5, 10, 15];
export const BATTERY_OPTIONS_PLATINUM = [0, 10, 15, 20, 30, 48];
...
export const defaultComponentPrices: ComponentPrices = {
  panelWattage: 650,
  panelPerUnit: 2_000_000,       // LONGi Hi-MO X10 650Wp (marketplace mode, before ongkir)
  mountingPerPanel: 400_000,     // Bracket Al + hardware + labor pasang
  carportPerKwp: 3_500_000,      // Kanopi carport per kWp (market mode)
  bosPerPanel: 275_000,          // Kabel DC/AC + MC4 + Tomzn MCB DC/AC per panel
  spdGroundingPerSystem: 1_500_000, // SPD + grounding + lightning arrester
  laborPerPanel: 350_000,        // Labor pasang panel surya
  laborInverterPerUnit: 1_500_000, // Labor instalasi inverter
  laborBatteryPerKwh: 200_000,   // Labor instalasi baterai
  batteryPerKwh: 1_300_000,      // Shoto LiFePO4 battery pack (marketplace mode)
  bosBatteryPerKwh: 200_000,     // Kabel baterai, fuse, protective per kWh
  surveyDesignFee: 2_000_000,    // Survei & desain
  commissioningFee: 1_000_000,   // Testing & commissioning
  logisticsPerPanel: 200_000,    // Ongkir per panel ke Jambi
  monitoringBasic: 2_500_000,
  monitoringStandard: 6_000_000,
  monitoringIndustrial: 18_000_000,
};

export const defaultInverterPrices: InverterPrices = {
  deye3k6: 6_500_000,       // Powmr SPF 3.6KW 24V/48V Hybrid MPPT
  deye6k: 10_000_000,       // Powmr SPF 5.5KW 48V Hybrid MPPT
  deye8k: 14_000_000,       // Powmr SPF 8KW 48V Hybrid MPPT
  growatt10k: 18_000_000,   // Powmr SPF 10.2KW 48V Hybrid MPPT
  deye10k3p: 22_000_000,    // Powmr SPF 12KW 48V Hybrid MPPT
  deye15k3p: 28_000_000,    // Powmr SPF 15KW 48V Hybrid MPPT
  deye20k3p: 38_000_000,    // Powmr SPF 20KW 48V Hybrid MPPT
};
...
export const inverterDisplayNames: Record<keyof InverterPrices, string> = {
  deye3k6: "POWMR SPF 3.6KW Hybrid MPPT",
  deye6k: "POWMR SPF 5.5KW Hybrid MPPT",
  deye8k: "POWMR SPF 8KW Hybrid MPPT",
  growatt10k: "POWMR SPF 10.2KW Hybrid MPPT",
  deye10k3p: "POWMR SPF 12KW Hybrid MPPT",
  deye15k3p: "POWMR SPF 15KW Hybrid MPPT",
  deye20k3p: "POWMR SPF 20KW Hybrid MPPT",
};
...
export const inverterMeta: Record<keyof InverterPrices, { capacity: number; phase: string; type: string }> = {
  deye3k6: { capacity: 3.6, phase: "1-Fase", type: "Hybrid MPPT" },
  deye6k: { capacity: 5.5, phase: "1-Fase", type: "Hybrid MPPT" },
  deye8k: { capacity: 8.0, phase: "1-Fase", type: "Hybrid MPPT" },
  growatt10k: { capacity: 10.2, phase: "1-Fase", type: "Hybrid MPPT" },
  deye10k3p: { capacity: 12.0, phase: "3-Fase", type: "Hybrid MPPT" },
  deye15k3p: { capacity: 15.0, phase: "3-Fase", type: "Hybrid MPPT" },
  deye20k3p: { capacity: 20.0, phase: "3-Fase", type: "Hybrid MPPT" },
};

// --- Package Definitions ---
// v4: All packages are BASE (no battery). Battery is selectable via batteryOptions.
...
export const packageSpecs: PackageSpec[] = [
  ...
  {
    name: "Silver 1 kWp",
    desc: "2 panel LONGi 650Wp + Powmr 3.6KW Hybrid MPPT. Starter untuk rumah kecil: lampu LED, TV, charger HP, kipas angin.",
    specs: "2× LONGi 650Wp | Powmr 3.6kW",
    panelCount: 2,
    inverterKey: "deye3k6",
    batteryKwh: 0,
    tier: "silver",
    popular: false,
    batteryOptions: [0, 3, 5],
  },
  {
    name: "Silver 2 kWp",
    desc: "4 panel LONGi 650Wp + Powmr 3.6KW Hybrid MPPT. Rumah kecil-menengah: lampu, TV, kipas angin, kulkas kecil.",
    specs: "4× LONGi 650Wp | Powmr 3.6kW",
    panelCount: 4,
    inverterKey: "deye3k6",
    batteryKwh: 0,
    tier: "silver",
    popular: false,
    batteryOptions: [0, 3, 5, 7],
  },
  {
    name: "Silver 3 kWp",
    desc: "5 panel LONGi 650Wp + Powmr 3.6KW Hybrid MPPT. Rumah menengah: AC 1 unit, kulkas, mesin cuci.",
    specs: "5× LONGi 650Wp | Powmr 3.6kW",
    panelCount: 5,
    inverterKey: "deye3k6",
    batteryKwh: 0,
    tier: "silver",
    popular: false,
    batteryOptions: [0, 5, 7],
  },
  {
    name: "Silver 5 kWp",
    desc: "8 panel LONGi 650Wp + Powmr 5.5KW Hybrid MPPT. Rumah keluarga besar: AC 2 unit, water heater, dispenser.",
    specs: "8× LONGi 650Wp | Powmr 5.5kW",
    panelCount: 8,
    inverterKey: "deye6k",
    batteryKwh: 0,
    tier: "silver",
    popular: true,
    batteryOptions: [0, 5, 7, 10],
  },
  ...
  {
    name: "Gold 7 kWp",
    desc: "11 panel LONGi 650Wp + Powmr 8KW Hybrid MPPT. Toko, ruko, kantor kecil, cold storage mini, bengkel.",
    specs: "11× LONGi 650Wp | Powmr 8kW",
    panelCount: 11,
    inverterKey: "deye8k",
    batteryKwh: 0,
    tier: "gold",
    popular: false,
    batteryOptions: [0, 5, 10, 15],
  },
  {
    name: "Gold 10 kWp",
    desc: "16 panel LONGi 650Wp + Powmr 10.2KW Hybrid MPPT. Gudang, restoran, hotel kecil, minimarket, clinic.",
    specs: "16× LONGi 650Wp | Powmr 10.2kW",
    panelCount: 16,
    inverterKey: "growatt10k",
    batteryKwh: 0,
    tier: "gold",
    popular: true,
    batteryOptions: [0, 10, 15, 20],
  },
  ...
  // ============================
  {
    name: "Platinum 13 kWp",
    desc: "20 panel LONGi 650Wp + Powmr 12KW Hybrid MPPT. Pabrik menengah, processing plant, warehouse besar.",
    specs: "20× LONGi 650Wp | Powmr 12kW",
    panelCount: 20,
    inverterKey: "deye10k3p",
    batteryKwh: 0,
    tier: "platinum",
    popular: true,
    batteryOptions: [0, 10, 20, 30, 48],
  },
  {
    name: "Platinum 20 kWp",
    desc: "32 panel LONGi 650Wp + Powmr 20KW Hybrid MPPT. Mega proyek, industrial complex, pabrik besar, hotel bintang 4.",
    specs: "32× LONGi 650Wp | Powmr 20kW",
    panelCount: 32,
    inverterKey: "deye20k3p",
    batteryKwh: 0,
    tier: "platinum",
    popular: false,
    batteryOptions: [0, 20, 30, 48],
  },
];
...
// --- Calculate Package With Battery ---

export function calculatePackageWithBattery(
  pkg: PackageSpec,
  batteryKwh: number,
  components?: ComponentPrices,
  inverters?: InverterPrices,
  settings?: PricingSettings
): { price: number; hpp: number; ppn: number; priceFormatted: string } {
  const c = components || loadComponentPrices();
  const inv = inverters || loadInverterPrices();
  const s = settings || loadSettings();

  const costPerPanel = c.panelPerUnit + c.mountingPerPanel + c.bosPerPanel + c.laborPerPanel + c.logisticsPerPanel;
  const batteryCostPerKwh = c.batteryPerKwh + c.laborBatteryPerKwh + c.bosBatteryPerKwh;
  const fixedPerSystem = c.spdGroundingPerSystem + c.surveyDesignFee + c.commissioningFee;
  const marginMultiplier = 1 + s.marginPct / 100;
  const ppnMultiplier = 1 + s.ppnPct / 100;
  ...
  return { price, hpp, ppn, priceFormatted: formatRp(price) };
}
...
export function calculateROI(
  ...
  const return25Year = cum25 - price; // Net profit (keuntungan bersih)
  const returnMultiplier = price > 0 ? (cum25 - price) / price : 0; // Net profit multiple (keuntungan bersih / investasi)
